package multiproject.server.human
...
class HumanBuilder : EntityBuilder<Human>() {
    ...
    override val fields: MutableMap<String, CommandArgumentDto> = mutableMapOf(
        "name" to CommandArgumentDto(
            name = "name",
            required = false,
            type = multiproject.udpsocket.dto.command.FieldType.STRING
        ),
        "fatness" to CommandArgumentDto(
            name = "fatness",
            required = false,
            type = multiproject.udpsocket.dto.command.FieldType.ENUM,
        ...
        "position" to CommandArgumentDto(
            ...
            required = true,
            nested = mapOf(
                "x" to CommandArgumentDto(name = "x", type = multiproject.udpsocket.dto.command.FieldType.INT),
                "y" to CommandArgumentDto(name = "x", type = multiproject.udpsocket.dto.command.FieldType.INT)
            )
        )
    )
